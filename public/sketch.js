...
function startScreen() {
  background(60);
  nameInputField = createInput().attribute('maxlength', 10).attribute('placeholder', 'Enter Your Name');
  nameInputField.position(width / 2 - 75, height / 2 - 60);
  nameInputField.size(150, 20);
...
function bonjour() {
  let userName = nameInputField.value().trim();
  if (userName === "") return;
...
class SnakeBody {
  ...
  eyes() {
    ...
    if (this.xSpeed > 0) {
      ...
      fill(255);
      text(nameInputField.value(), this.x + 10, this.y + 30);
      ...
      fill(255);
      text(nameInputField.value(), this.x + 10, this.y - 5);
    } else if (this.ySpeed > 0) {
      fill(0);
